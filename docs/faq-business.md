# MAGS: Common Business and Conceptual Questions

*Frequently asked questions collected from customer conversations, workshops, and demonstrations. Answers are kept generic — no specific vendor or scenario references — so they apply across industries and use cases.*

*For technical implementation questions, see the [Technical FAQ](./faq.md). For detailed terminology, see the [Glossary](./Glossary.md).*

---

## Contents

1. [What Is It — Positioning and Comparison](#what-is-it--positioning-and-comparison)
2. [Trust, Safety, and Risk](#trust-safety-and-risk)
3. [Technical Capabilities and Limitations](#technical-capabilities-and-limitations)
4. [Implementation and Maintenance](#implementation-and-maintenance)
...
MAGS is the decision-making layer that sits between your operational data and the actions that data should trigger. It turns observations into decisions, and decisions into actions.
...
MPC is a control algorithm that uses a process model to predict future behaviour and optimise setpoints over a rolling time horizon. It is fast, mathematically rigorous, and well-proven. MAGS differs in four ways:
...
2. **Explainability**: MPC produces an optimal setpoint trajectory from a mathematical optimisation — it cannot explain the recommendation in business terms. MAGS produces a decision with documented reasoning.
3. **Model dependency**: MPC requires a calibrated, maintained process model. If the model drifts, MPC performance degrades silently. MAGS agents reason from domain knowledge and are explicit about uncertainty when their predictions don't match observations.
4. **Economic integration**: MPC optimises process performance (minimise deviation, minimise move effort). MAGS optimises business outcomes (maximise value per hour) subject to process and safety constraints. These are different objectives.

**The honest positioning**: MPC and MAGS are complementary, not competitive. MPC handles the fast control layer (seconds to minutes). MAGS handles the slower economic and strategic optimisation layer (minutes to hours). MAGS can function as the supervisor that determines the setpoints MPC then tracks.
...
A digital twin models the process. MAGS uses the digital twin as a data source and adds the decision-making layer on top. The digital twin becomes one of the inputs the Monitor agent uses to assess current state — it does not replace the twin.
...
**Containment**: Every action is within pre-approved operating bands. Even if the agent's reasoning is wrong, the worst outcome is a setpoint moved within the range your engineers signed off. Safety instrumented systems provide the final backstop — agents cannot defeat safety interlocks.
...
If data quality is compromised — stale reading, suspect instrument, failed analyser — the agent flags it and falls back to conservative operation rather than making decisions on uncertain data. The agent is explicit about its uncertainty and why it is holding.

The design principle: fail safe. When in doubt, escalate to a human. Agents are designed to be more conservative than an operator when uncertain, not less.
...
### What are the cybersecurity implications of giving AI agents write access to the control system?

Agents write to existing controllers through existing integration layers — they do not bypass the control system or communicate directly with field devices. All communication goes through standard industrial protocols (OPC-UA, REST, MQTT) using the same security as existing historian and monitoring connections.

Write access is scoped to specific, pre-authorised outputs only. Every write is logged with timestamp, value, and approval reference. Agents cannot modify controller tuning, modes, or configurations — outputs only, within defined bounds.
...
For organisations with strict OT security requirements: agents can be configured in advisory-only mode where they never write to the control system. This eliminates write-access concerns entirely while retaining monitoring and advisory value.
...
- **Staleness detection**: If a sensor or analyser has not updated within its expected cycle time, it is flagged as stale.
...
- **Cross-validation**: Where multiple measurements should be consistent (e.g., material balances), inconsistencies are flagged.
...
This is more rigorous than typical practice in many operations, where a stale reading may not be noticed until the next manual round.
...
For pure event detection — threshold breaches, rate-of-change alerts — Data Streams with hardcoded rules or lightweight models are faster, cheaper, and more reliable than agents. Agents are the wrong tool for detection.
...
- The objective is explicit (e.g., a defined value formula)
...
### What happens to data that arrives while an agent is busy processing?
...
**Observe-Reflect-Plan-Act.** The agent continuously observes data, synthesises observations into insights (reflections), creates plans based on those insights, and acts on approved plans. This cycle runs continuously, 24/7, without fatigue or distraction.
...
The technical deployment is fast. The change management — building operator trust, getting engineering sign-off on operating bands, getting IT/OT security approval — is the pacing factor.
...
| Economic layer | Month 3–6 | Continuous financial calculation adds the quantified case for each recommendation. Savings visible in reports. |
...
This is a fair concern — many AI systems become maintenance nightmares when the process changes.
...
None of these require retraining a model or rebuilding the system from scratch. Every agent's rules, objectives, and data requirements are explicit and human-readable — you can see exactly what each agent is doing and why.
...
Existing AI systems, historians, digital twins, process simulators, and ERP systems all become data sources or tools that agents can call. Integration is handled once in the Data Stream layer and then made available to all agents that need it.
...
The key insight is **separation of duties**. The agent that proposes a profitable action must never be the same agent that approves it — this is the same principle as requiring two signatures for financial transactions, or separating design from inspection in safety engineering.

A Guardian agent's entire value comes from having no incentive to relax constraints for economic gain. A single AI with conflicting objectives cannot provide this structural guarantee. One model trying to do everything will always face pressure to trade safety margin for operational benefit.
...
One profile can have many instances — for example, one "Process Monitor" profile deployed across ten different units, each with the same role and rules but monitoring different equipment. Creating a separate profile per instance is the most common architectural mistake: it results in maintenance overhead that scales with the number of units rather than the number of distinct roles.
...
An **observation** is what the agent sees in the data at a moment in time.

A **reflection** is what the agent concludes from multiple observations over time — the synthesised insight that can inform future decisions. Reflections represent accumulated learning and are what allow agents to improve their decision-making as they gain operational experience.
...
A **utility function** says "I prefer this" — it guides optimisation toward better outcomes. It creates a gradient that steers decisions toward higher-value states.

A **constraint** says "I forbid that" — it gates optimisation and rejects any plan that violates it, regardless of how attractive the plan is on other dimensions. Plans that violate constraints are rejected before they can be executed.

For safety-critical measures, use both: a constraint as the hard floor that can never be crossed, and a utility function as the gradient that steers decisions away from the floor before it is ever reached.
...
Productive tension between agent objectives is a feature, not a bug. An economic agent optimising for value and a guardian agent optimising for constraint compliance create the same checks and balances expected in any well-governed engineering process.
...
*This document is maintained as a living collection. When a question comes up repeatedly in customer conversations and the answer is not already here, add it. Open an issue or submit a pull request with the question, a generic answer, and links to supporting documentation.*
