...
echo ""
print_color "$CYAN" "PostgREST Configuration:"
print_color "$GRAY" "PostgREST exposes the 'api' schema as a REST API on port 3000."

read -p "Enter PostgREST HTTP port (default: 3000): " postgrest_port_choice
if [[ "$postgrest_port_choice" =~ ^[0-9]+$ ]]; then
    POSTGREST_PORT=$postgrest_port_choice
else
    POSTGREST_PORT=3000
fi

read -p "Enter PostgREST HTTPS port (default: 3443): " postgrest_https_port_choice
if [[ "$postgrest_https_port_choice" =~ ^[0-9]+$ ]]; then
    POSTGREST_HTTPS_PORT=$postgrest_https_port_choice
else
    POSTGREST_HTTPS_PORT=3443
fi

echo ""
print_color "$CYAN" "CORS Configuration:"
print_color "$GRAY" "  Enter the origin(s) allowed to call PostgREST from the browser."
print_color "$GRAY" "  Examples: https://app.example.com  or  *  (allow all, dev only)"
read -p "Allowed CORS origins (default: *): " cors_choice
if [ -n "$cors_choice" ]; then
    POSTGREST_CORS_ORIGINS="$cors_choice"
else
    POSTGREST_CORS_ORIGINS="*"
fi

echo "Generating PostgREST secrets..."
POSTGREST_AUTHENTICATOR_PASSWORD=$(openssl rand -base64 24 | tr -d "=+/" | cut -c1-32)
# JWT secret must be at least 32 characters; generate 48 to be safe
PGRST_JWT_SECRET=$(openssl rand -base64 36 | tr -d "=+/" | cut -c1-48)
print_color "$GREEN" "Generated authenticator password and JWT secret"

cat >> .env << EOF

# PostgREST Configuration
POSTGREST_PORT=$POSTGREST_PORT
POSTGREST_HTTPS_PORT=$POSTGREST_HTTPS_PORT
...
PGRST_DB_URI=postgres://authenticator:${POSTGREST_AUTHENTICATOR_PASSWORD}@timescaledb:5432/${DATABASE_NAME}
PGRST_DB_SCHEMAS=api
PGRST_DB_ANON_ROLE=anon
PGRST_JWT_SECRET=$PGRST_JWT_SECRET
PGRST_DB_MAX_ROWS=1000
PGRST_SERVER_CORS_ALLOWED_ORIGINS=$POSTGREST_CORS_ORIGINS
PGRST_DB_POOL=10
PGRST_DB_PREPARED_STATEMENTS=true
EOF
print_color "$GREEN" "PostgREST configuration written to .env"
...
echo "Configuring PostgREST initialization script..."
POSTGREST_SQL="init/02-init-postgrest.sql"
if [ -f "$POSTGREST_SQL" ]; then
    sed -i "s/{{AUTHENTICATOR_PASSWORD}}/${POSTGREST_AUTHENTICATOR_PASSWORD}/g" "$POSTGREST_SQL"
    print_color "$GREEN" "PostgREST initialization script configured"
else
    print_color "$YELLOW" "Warning: $POSTGREST_SQL not found - creating it"
    cat > "$POSTGREST_SQL" << SQLEOF
-- PostgREST Database Setup (generated by install.sh)
...
BEGIN
    ...
    ELSE
        ALTER ROLE authenticator PASSWORD '${POSTGREST_AUTHENTICATOR_PASSWORD}}';
    END IF;
END
\$\$;
...
SQLEOF
    print_color "$GREEN" "Created PostgREST initialization script"
fi
...
EOF
    ...
    echo "Creating nginx configuration for PostgREST HTTPS..."
    cat > postgrest/nginx.conf << 'EOF'
events {
    worker_connections 1024;
}
...
EOF
    sed -i "s/DOMAIN_PLACEHOLDER/$DOMAIN/g" postgrest/nginx.conf
    print_color "$GREEN" "Created nginx.conf for PostgREST HTTPS (port $POSTGREST_HTTPS_PORT)"
fi
...
echo ""
print_color "$CYAN" "PostgREST REST API Details:"
echo "  URL (HTTP):  http://localhost:$POSTGREST_PORT"
if [ "$ENABLE_SSL" = true ]; then
    echo "  URL (HTTPS): https://localhost:$POSTGREST_HTTPS_PORT"
fi
print_color "$GRAY" "  Exposes: api schema as REST endpoints"
print_color "$YELLOW" "  JWT Secret:              $PGRST_JWT_SECRET"
print_color "$YELLOW" "  Authenticator password:  $POSTGREST_AUTHENTICATOR_PASSWORD"
print_color "$GRAY" "  CORS origins: $POSTGREST_CORS_ORIGINS"
print_color "$GRAY" "  OpenAPI schema: GET http://localhost:$POSTGREST_PORT/"
print_color "$CYAN" "  Tiger Cloud: set PGRST_DB_URI in .env to your Timescale Cloud connection string"
echo ""
...
echo "  - init/01-init-timescaledb.sql (TimescaleDB initialization)"
echo "  - init/02-init-postgrest.sql (PostgREST roles and api schema)"
...
[ "$ENABLE_SSL" = true ] && echo "  - postgrest/nginx.conf (PostgREST HTTPS proxy)"
